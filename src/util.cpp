...
#else
    const char* pszModule = "UniversalRoyalCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UniversalRoyalCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UniversalRoyalCoin
    // Mac: ~/Library/Application Support/UniversalRoyalCoin
    // Unix: ~/.UniversalRoyalCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UniversalRoyalCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "UniversalRoyalCoin";
#else
    // Unix
    return pathRet / ".UniversalRoyalCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "UniversalRoyalCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "UniversalRoyalCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong UniversalRoyalCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("UniversalRoyalCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
