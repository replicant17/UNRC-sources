...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("UniversalRoyalCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #UniversalRoyalCoinTEST2\r");
            Send(hSocket, "WHO #UniversalRoyalCoinTEST2\r");
        } else {
            // randomly join #UniversalRoyalCoin00-#UniversalRoyalCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #UniversalRoyalCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #UniversalRoyalCoin%02d\r", channel_number).c_str());
        }
